...
const { GRUPLAR, grupNormalize } = require('./gruplar');
...
class Cekirdek {
    ...
    setSec(setId) {
        ...
        this.aktifGrup = grupNormalize(set.grup);   // grup kilidi setten otomatik (eski i/c → u)
    ...
    katil(playerId, kod) {
        ...
        if (!this.karma && grupNormalize(o.grup) !== this.aktifGrup && !o.misafir)
        ...
        const yeni = !p;
        if (yeni) {
            p = this.players[playerId] = yeniOyuncu(kod, o.isim, grupNormalize(o.grup), o.misafir);
