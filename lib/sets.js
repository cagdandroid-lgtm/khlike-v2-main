...
function dogrula(set, id) {
    ...
    if (!grupNormalize(set.grup)) hatalar.push('grup p/e/u olmalı');   // eski i/c değerleri u kabul edilir
...
function yukle() {
    ...
    fs.readdirSync(SETS_DIR).filter(f => f.endsWith('.json')).sort().forEach(f => {
        ...
        try {
            ...
            if (h.length) sorunlar.push({ id, hatalar: h }); else { set.grup = grupNormalize(set.grup); setler[id] = set; }
...
function kaydet(id, set) {           // panelden içe aktarım / düzenleme
    ...
    set.grup = grupNormalize(set.grup);
